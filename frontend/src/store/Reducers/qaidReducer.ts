import { Qaid, QaidState } from "@/types/qaidTypes";
import {
    ApiPagination,
...
export const createQaid = createAsyncThunk<
  ApiResponse,
  QaidState,
...
export const qaidReducer = createSlice({
  name: "qaid",
  initialState,
  reducers: {
    messageClear: (state) => {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(createQaid.pending, (state) => {
        state.loader = true;
      })
      .addCase(createQaid.rejected, (state, { payload }) => {
        state.loader = false;
        state.errorMessage = payload?.error || "Registration failed";
      })
      .addCase(createQaid.fulfilled, (state, action) => {
        const { message,qaidVoucherNo } = action.payload;


        return {
          ...state,
          loader: false,
             qaidVoucherNo: qaidVoucherNo || 0,
...
export const { messageClear } = qaidReducer.actions;
export default qaidReducer.reducer;
