...
const IncomeTransferstList: React.FC = () => {
  ...
  const { incomeTransfers, totalTransfers, successMessage, errorMessage } =
  ...
  const handleDeleteIncomeTransfer = (voucherNo: number) => {
    const incomeTransferToDelete = incomeTransfers.find(
      (p) => p.voucherNo === voucherNo,
    );

    if (!incomeTransferToDelete) {
      toast.error("Payment not found");
      return;
    }

    if (!incomeTransferToDelete.fiscalYear) {
    ...
    showConfirmation(t("sendTransfer.are_you_sure_to_cancel"), () => {
      dispatch(
        deleteIncomeTransfer({
          voucherNo,
          fiscalYear: incomeTransferToDelete.fiscalYear,
          typeId: FORM_TYPES.INCOMETRANSFER,
          deleteIncomeTransfer: incomeTransferToDelete,
        }),
      );
    });
  };
    ...
    <div>
      <div className="px-2 pb-5 lg:px-4">
        <div className="flex items-center justify-between pb-3">
          ...
          <span className="px-2">
            {totalTransfers} {t("sendTransfer.income_transfer")}
          </span>
        </div>
          ...
          <div className="flex flex-col gap-2 pb-3 lg:flex-row lg:items-center lg:justify-between lg:gap-1">
            ...
            <div className="flex w-full flex-col items-center  gap-1 lg:w-auto">
              <div className="flex w-full items-center gap-3  lg:w-auto">
                <label className="px-4 text-sm" htmlFor="currencyId">
                ...
                <select
                  ...
                  id="currencyId"
                  className="w-full rounded-md border border-slate-400 bg-white px-6 py-[2px]  text-sm text-black focus:border-secondary lg:w-auto"
                  // required
                >
               ...
               {/* <div className="flex w-full items-center gap-3  lg:w-auto">
                 <label className="px-[5px] text-sm lg:px-[2px]" htmlFor="currencyId">
                  {t("sendTransfer.paid_type")}
                </label>
                <select 
    value={paidId === undefined ? '' : paidId.toString()} 
    onChange={(e) => setPaidId(e.target.value === '' ? undefined : parseInt(e.target.value))}
     id="paidId"
  ...
  >
    <option value="">{t("sendTransfer.select_all")}</option>
    <option value="1">{t("sendTransfer.paid")}</option>
    <option value="0">{t("sendTransfer.unpaid")}</option>
  </select>
              </div> */}
            </div>
            ...
            <div className="flex w-full items-center gap-3 lg:w-auto">
              ...
              <input
                ...
                value={searchValue}
                className="w-full rounded-md border text-sm border-[#bcb9b9] bg-[#F9FBFE] px-4 py-1 text-[#5c5a5a] outline-none focus:border-[#bcb9b9] lg:w-auto"
                type="text"
                id="search"
                name="search"
                placeholder="search"
                      ...
                      <td className="whitespace-nowrap px-4  py-2">
                        <div className="flex items-center justify-end gap-2">
                          {o.paidId===0?(
                            <>
                            <Link
                            to={`/admin/dashboard/paid-income-transfer/${o.fiscalYear}/${o.voucherNo}?payType=1`}
                            className={`rounded bg-darkGreen px-[8px] py-[3px] text-sm  text-white hover:bg-semiLightGreen hover:text-black shadow-md hover:shadow-lg hover:shadow-[#2a629aab]`}
                          >
                            {t("paymentS.payment")}
                          </Link>
                          <Link
                            to={`/admin/dashboard/paid-income-transfer/${o.fiscalYear}/${o.voucherNo}?payType=2`}
                            className={`rounded bg-yellow-400  px-[8px] py-[3px] text-sm  text-black hover:bg-yellow-700 hover:text-white shadow-md hover:shadow-lg hover:shadow-[#2a629aab]`}
                          >
                            {t("sendTransfer.pay_to_account")}
                          </Link>
                          <Link
                            to={`/admin/dashboard/income-transfers/${o.fiscalYear}/${o.voucherNo}`}
                          ...
                          <button
                            onClick={() =>
                              handleDeleteIncomeTransfer(o.voucherNo)
                            }
                          ...
                          ):(<span className="text-green-700 font-semibold">{t("sendTransfer.transfer_paid")}</span>)}
...
export default IncomeTransferstList;
