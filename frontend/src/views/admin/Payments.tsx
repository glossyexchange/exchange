...
const { user } = useAuth();
  ...
  const { successMessage, errorMessage, loader, newVoucherNo, paymentId } = useSelector(
    ...
    const loadVoucher = async () => {
      try {
        const result = await dispatch(
          getPayment({ voucherNo, fiscalYear }) 
        ).unwrap();
...
const handleSearchVoucher = async () => {
  ...
  try {
   const result = await dispatch(
  getPayment({ voucherNo, fiscalYear: searchFiscalYear })
).unwrap();
    ...
    if (result.payment) {
      // Populate form – same as in URL loading
      const paymentData = result.payment;
      setIsEditing(true);
      setIsFormEnabled(false);
      setEditingPaymentId(paymentData.id);
...
const handleModalAccountSelect = async (account: AccountGet) => {
  ...
  // Also dispatch to get movements directly
  dispatch(
    getMovementsByAccount({
...
const searchAccountById = async (accountId: number) => {
  ...
  const add = (e: React.FormEvent) => {
    ...
    const paymentsData = {
      paymentTypeId: FORM_TYPES.PAYMENT,
      currencyId: state.currencyId,
      ...
      createdAt,
      receiptNo: 0,
      debtorId: customerState.accountId,
      daneId: accountTypeId.QASA,
      typeId: FORM_TYPES.PAYMENT,
      type: "پارەدان/دفع",
    };
...
if (isEditing) {
  ...
  const updateData: UpdatePaymentData = {
    // Identifiers
    voucherNo,
    paymentId: editingPaymentId || paymentId,
    ...
    debtorId: customerState.accountId,
    daneId: accountTypeId.QASA,
    // type: "پارەدان",   // if you really need to edit this (rare)
  };

  dispatch(updatePayment(updateData)).then(() => {
    // setIsFormEnabled(false);
      ...
      dispatch(createPayment(paymentsData)).then(() => {
        // setIsFormEnabled(false);
        setVoucherSearch("");
      });
    }
  ...
  />
          ...
          <div className="flex justify-end">
            ...
            <Link
              to="/admin/dashboard/payment-list"
                ...
                ref={registerRef(0)}
                  ...
                  onChange={(e) => {
                    ...
                    const selectedText =
                      ...
                      setState({ ...state, currencyId: value, currencyType:selectedText });
