...
import { Payments } from "@/types/paymentType";
...
const PaymentList: React.FC = () => {
  ...
  const { successMessage, errorMessage, payments, totalPayments } = useSelector(
...
const handleDeletePayment = (payment: Payments) => {
  if (window.confirm(t("currencyS.delete_confirm") || "Are you sure?")) {
    dispatch(
      deletePayment({
        paymentId: payment.id,     
        formType: FORM_TYPES.PAYMENT,
  ...
  return (
    <div className="px-2 pb-5 lg:px-4">
      <div className="flex items-center justify-between pb-3">
        ...
        <span className="px-2">
          {totalPayments} {t("paymentS.payment")}
        </span>
      </div>
            ...
            >
              <thead className="border-b border-[#dcdada] bg-[#EEF2F7] text-xs uppercase text-[#5c5a5a]">
                <tr>
                  ...
                  <th className="px-4 py-2">{t("orderS.voucher_no")}</th>
                  <th className="px-4 py-2">{t("orderS.customer_name")}</th>
                  <th className="px-4 py-2">{t("categoryS.phone")}</th>
                  <th className="px-4 py-2">{t("paymentS.receipt_amount")}</th>
              ...
              <tbody>
                {payments?.map((p, i) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-4 py-1">{p?.payer}</td>
                    <td className="whitespace-nowrap px-4 py-1">
                      {p?.payerPhone}
                    </td>
                    <td className="whitespace-nowrap px-4 py-1 font-semibold">
                      {p.totalAmount.toLocaleString("en-IQ")}{" "}
                    </td>
                    <td className="whitespace-nowrap px-4 py-1">
                      {p.currencyType}{" "}
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-1">
                      <div className="flex items-center justify-end gap-3">
                        <Link
                          to={`/admin/dashboard/payment/${p.fiscalYear}/${p.voucherNo}`}
